...
class Student extends User {
	...
	function register($email) {
		try {
			...
			if ($conn->query($sql)) {
				$sql = "SELECT file.id as id FROM file INNER JOIN deliverable ON file.id = deliverable.file WHERE deliverable.school = ".$this->getSchool("code")." AND deliverable.praxis = ".$this->getPraxis()." ORDER BY file.id";
				$result = $conn->query($sql);
...
class Upload {
	...
	function getSchoolUploadedFiles($school,$praxis,$period) {
		try {
			$database = new DB();
			$conn = $database->connect();
      ...
      echo '<div class="shadow p-4 mb-4 bg-white">';
      	...
      	echo '<!-- Nav tabs -->
				...
				echo '<!-- Nav tabs -->
							<ul class="nav nav-tabs">
  						<li class="nav-item">
    						<a class="nav-link" data-toggle="tab" href="#home">Δικαιολογητικά Έναρξης ΠΑ</a>
  						</li>
  						<li class="nav-item">
    						<a class="nav-link active" data-toggle="tab" href="#menu1">Παραδοτέα Έγγραφα Λήξης ΠΑ</a>
  						</li>
						</ul>';
